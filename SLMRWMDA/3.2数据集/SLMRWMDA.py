...
a = np.mat(np.zeros((788, 374)))  # 定义一个495*383的0矩阵
AA = a.astype(int)  # 把矩阵mta化为整形

# 读取miRNA-disease关联数字编
b = np.loadtxt('miRNA-disease_association.txt')
B = b.astype(int)
...
# 读取疾病语义类似性加权矩阵
c = np.loadtxt('disease_semantic_sim_加权.txt')
# C = 0.5 * (c1 + c2)  # 语义类似性矩阵
C = np.loadtxt("disease_semantic_sim.txt")

# 读取miRNA功能类似性矩阵
D = np.loadtxt('miRNA_similarity_new.txt')

# 读取miRNA功能类似性加权矩阵
d = np.loadtxt('miRNA_similarity_new_加权.txt')
...
for count in range(1):
    ...
    array = np.random.permutation(8968)
    for y in range(5):
        A = AA.copy()
        if y < 4:
            array1 = array[0 + y * 1794:1794 * (y + 1)]
        else:
            print("第", str(y+1), "次")
            array1 = array[0 + y * 1794:1794 * (y + 1)-2]
        for z1 in array1:
            A[B[z1, 0] - 1, B[z1, 1] - 1] = 0  # 将数据分成五份，其中一份作为验证集

        alpha = 0.1
        J = np.mat(np.zeros((374, 374)))
        X = np.mat(np.zeros((374, 374)))
        E = np.mat(np.zeros((788, 374)))
        Y1 = np.mat(np.zeros((788, 374)))
        Y2 = np.mat(np.zeros((374, 374)))
        mu = 10 ** -4
        max_mu = 10 ** 10
        rho = 1.1
        epsilon = 10 ** -6
        ...
        print('第' + str(y + 1) + '次循环')
        #    print('miRNA=%d,disease=%d'%(i+1,j+1))
        #    come=1
        while True:
            ...
            ATA = A.T * A
            X = (ATA + np.eye(374)).I * (ATA - A.T * E + J + (A.T * Y1 - Y2) / mu)
            temp1 = A - A * X
        ...
        gamaD1 = gamad1 * 374 / sum1  # 计算参数gamaD1
        KD = np.mat(np.zeros((374, 374)))  # 定义一个383*383的0矩阵，初始化疾病之间高斯相似性矩阵
        for ab in range(374):
            for ba in range(374):
        ...
        gamaD2 = gamad2 * 788 / sum2  # 计算参数gamaD1
        KM = np.mat(np.zeros((788, 788)))  # 定义一个495*495的0矩阵，初始化miRNA之间高斯相似性矩阵
        for cd in range(788):
            for dc in range(788):
        ...
        RM = np.mat(np.zeros((788, 788)))
        MM = np.mat(np.zeros((788, 788)))
        M0 = np.mat(np.zeros((788, 1)))
        for MM1 in range(788):
        ...
        RD = np.mat(np.zeros((374, 374)))
        DD = np.mat(np.zeros((374, 374)))
        D0 = np.mat(np.zeros((374, 1)))
        for DD1 in range(374):
        ...
        # 把SD规范化
        SD1 = RD.copy()
        for nn1 in range(374):
            for nn2 in range(374):
                #            sum3=0
                #            sum4=0
                #            for dd in range(383):
        ...
        SM1 = RM.copy()
        for mm1 in range(788):
            for mm2 in range(788):
                #            sum5=0
                #            sum6=0
                #            for mm in range(495):
                #                sum5=sum5+SM1[mm1,mm]
                #                sum6=sum6+SM1[mm2,mm]
